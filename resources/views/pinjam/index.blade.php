...
<div class="col-md-12">
    <div class="card card-info">
        <div class="card-header">
            ...
            <a type="button" class="btn btn-success float-right" href="{{ route('pinjam.create') }}">
            ...
            <table id="example1" class="table table-bordered table-striped ">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $dt->peminjam->nama_peminjam }}</td>
                        <td>{{ $dt->user->nama_user }}</td>
                        <td>{{ $dt->barang->nama_barang }}</td>
                        <td>{{ $dt->jumlah }}</td>
                        <td>{{ $dt->tanggal_peminjam }}</td>
                        <td>{{ $dt->tanggal_kembalian }}</td>
                        <td>{{ $dt->barang->kondisi }}</td>
                        <td>{{ $dt->keterangan }}</td>
                        <td>
                            <div class="btn-group">
                                <form action="{{ route('pinjam.destroy',$dt->id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
